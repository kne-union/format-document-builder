...
const InputField = ({
                        onChange, triggerValidate, value, isActive, style, className, ...props
                    }) => {
    useEffect(() => {
...
const TextAreaField = ({
                           onChange, triggerValidate, value, style, isActive, className, ...props
                       }) => {
    ...
    return (<textarea
        ...
        ref={ref}
        style={Object.assign({}, style, {
            border: "none", outline: "none", display: "block", resize: "none",
        }, calcStyle)}
        className={classnames(className, "react-form__field-component")}
        value={value || ""}
        onChange={(e) => {
...
const output = {
    Input: withField(InputField), TextArea: withField(TextAreaField),
};
